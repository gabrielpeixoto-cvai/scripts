...
" Abbreviations
" ------------------------------------------------------------
"iab me Nagib Coelho Matni Neto
"iab @@  nmatni at gmail dot com

" atalhos para java
"
"iab sout System.out.println(" <esc>xll


"Atalhos para C
"
"iab forc for( <left>i=0;i<;i++<right><right>{<esc>hhhhhhi

"ab pr print("
"Atalhos para PHP e mysql
"iab pfor for( <left>$i=0;$i<;$i++<right><right>{<esc>hhhhhhhh i
"iab pwh while(<right>{<cr><cr><esc>kklllllli
"iab prr  print_r( <backspace>$
"iab myac mysql_fetch_assoc( <backspace>$
"iab myqy mysql_query( <backspace>$
"
...
" por,  dependendo do tipoe de arnuivonir  as várias
...
" Para chamar as funções de inserção no cabeçao 
...
"   call append(1, "# Criado em:" . strftime("%a %d/%b/%Y hs %H:%M"))
   call append(1, "# Author:" . " " . strftime("%b %Y"))
   call append(2, "# Cassio Trindade Batista - cassio.batista.13@gmail.com")
   normal($)
endfun
map <F3> :call InsertHeadBash()<cr>
...
function! InsertHeadPy()
   ...
   call append(5, "@org:")
   call append(6, "@author: Cassio Trindade Batista(cassio.batista.13@gmail.com) \"\"\"")
   normal($)
endfun
map <F5> :call InsertHeadPy()<cr>
"Recarrega o vim
